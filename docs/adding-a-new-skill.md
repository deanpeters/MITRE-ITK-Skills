...
This writes `claude-skills/itk-<alias>.md`. If the slug is long (more than ~30 characters), add a shorter alias to the `SLUG_ALIASES` dict in `scripts/generate-skills.py` before running.
...
| `claude-skills/itk-*.md` | Derived from SKILL.md | "How to Help" pattern is fixed across all skills |
